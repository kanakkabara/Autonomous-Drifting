...
from network_models import DQNetwork, QNetwork
...
class QAgent():
    def __init__(self, sess, config):
        self.primary_Q_network = QNetwork(
            config.learning_rate,
            config.s_size,
            config.a_size,
            config.h_size,
    ...
    def update_agent(self):
        ...
        # max_next_state_action = self.sess.run(self.primary_Q_network.action_predicted, feed_dict={
        #     self.primary_Q_network.state_input: np.vstack(train_batch[:, 3])})
        # target_network_Q_values = self.sess.run(self.primary_Q_network.Qout, feed_dict={
        #     self.primary_Q_network.state_input: np.vstack(train_batch[:, 3])})
        ...
        max_next_state_Q = np.max(self.sess.run(self.primary_Q_network.Qout, feed_dict={
            self.primary_Q_network.state_input: np.vstack(train_batch[:, 3])
        }), axis=1)

        end_multiplier = (1 - train_batch[:, 4])
        target_Q_values = train_batch[:, 2] + \
            (self.gamma * max_next_state_Q * end_multiplier)
